...
def angle_adj(u0, a, b, c, ang_1, ang_2, ang_3, ang_4, w, times):
    for i in range(times):
        ...
        # k矩陣的單位為弧度
        k = p * np.array([
            (ang_1 / p)  - (math.atan((b['x'] - a['x'])/(b['y'] - a['y'])) - math.atan((u0['x'] - a['x']) / (u0['y'] - a['y'])) + 0)
           ,(ang_2 / p)  - (math.atan((u0['x'] - b['x'])/(u0['y'] - b['y'])) - math.atan((a['x'] - b['x']) / (a['y'] - b['y'])) + 0)
           ,(ang_3 / p)  - (math.atan((c['x'] - b['x'])/(c['y'] - b['y'])) - math.atan((u0['x'] - b['x']) / (u0['y'] - b['y'])) + math.pi )
           ,(ang_4 / p)  - (math.atan((u0['x'] - c['x'])/(u0['y'] - c['y'])) - math.atan((b['x'] - c['x']) / (b['y'] - c['y'])) + 0)
        ])
        ...
        s_obs:{}
        ql:{}
        '''.format(round(a_u0, 6), round(b_u0, 6), round(c_u0, 6), np.round(j, 6), np.round(k, 6), np.round(x, 6)
        , round(n_x_u, 6), round(n_y_u, 6), np.round(v, 6), round(s0, 6), np.round(adj_angle, 6), round(s_xu, 6), round(s_yu, 6)
        ,round(s_u, 6), np.round(s_obs, 6), np.round(q_l, 6)))
        u0 = {'x': n_x_u, 'y': n_y_u}
